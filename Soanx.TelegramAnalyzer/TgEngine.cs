...
public interface ITgEngine {
    void SubscribeToUpdateReceivedEvent();
    Task HandleAuthentication();
}
public class TgEngine : ITgEngine {
    ...
    public async Task FormilizeMessages(List<TgMessage> tgMessages) {
        if(openAiApiClient == null) {
            ...
            openAiApiClient = new ApiClient(openAiParameters);
        }
